...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // ────────────────────────────────────────
    // POST /care-profiles (admin)
    // ────────────────────────────────────────
    if (method === "POST" && segments[0] === "care-profiles" && !segments[1]) {
      const adminId = await requireAdmin(req, supabaseUrl, anonKey, h, requestId);
      const body = await req.json();
      const v = validate(careProfileBodySchema, body, h);
      if (v.error) return v.error;

      log.info("Admin create care profile", { adminId, plant_id: v.data.plant_id });

      const { data, error } = await serviceClient
        .from("species_care_profiles")
        .insert({ ...v.data, moderation_status: "approved", moderated_by: adminId, moderated_at: new Date().toISOString() })
        .select()
        .single();
    ...
    // ────────────────────────────────────────
    // PUT /care-profiles/:id (admin)
    // ────────────────────────────────────────
    if (method === "PUT" && segments[0] === "care-profiles" && segments[1]) {
      const profileId = segments[1];
      const uuidResult = schemas.uuid.safeParse(profileId);
      if (!uuidResult.success) throw new AppError("Invalid profile ID", 400, "INVALID_ID");

      const adminId = await requireAdmin(req, supabaseUrl, anonKey, h, requestId);
      const body = await req.json();
      const v = validate(careProfileUpdateSchema, body, h);
      if (v.error) return v.error;

      log.info("Admin update care profile", { adminId, profileId });

      const { data, error } = await serviceClient
        .from("species_care_profiles")
        .update({ ...v.data, moderated_by: adminId, moderated_at: new Date().toISOString() })
        .eq("id", profileId)
        .select()
        .single();
    ...
    // ────────────────────────────────────────
    // DELETE /care-profiles/:id (admin)
    // ────────────────────────────────────────
    if (method === "DELETE" && segments[0] === "care-profiles" && segments[1]) {
      const profileId = segments[1];
      const uuidResult = schemas.uuid.safeParse(profileId);
      if (!uuidResult.success) throw new AppError("Invalid profile ID", 400, "INVALID_ID");
      ...
      log.info("Admin delete care profile", { adminId, profileId });
